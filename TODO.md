...
- [x] `check-portable-commands.sh` test cases. Lives at `tests/check-portable-commands.bats`; 16 tests covering all rows of the per-case table in `docs/testing.md` (parse-check, command position after a separator, at the start, after variable assignments, after `xargs` with options, after `find -exec`, by absolute path, on a later line; the preferred tool, a prose mention, an argument, and a word prefix not flagged; the two inert configurations; missing-field and malformed-JSON canaries). The active/inert decision depends on which tools are on PATH, so `setup()` builds a PATH from scratch: stub `sed` and `gsed` executables plus symlinks to the real `bash`, `jq`, `grep`, `realpath`, `rm`, and `ln`.
...
- [x] Add `check-portable-commands.sh` Bash `PreToolUse` hook (2026-09-11). Denies a Bash command that invokes, in command position, a platform-native tool listed in a native/preferred table (seeded with `sed`/`gsed`; further BSD-versus-GNU pairs are present as commented entries), when the preferred implementation is installed and is a different binary from the native one, and names the replacement in the deny reason. An entry is inert where the preferred tool is absent or is the same binary, so the script is safe on GNU systems. Hand-run payload cases all pass (see the Tests item); the one known false positive is a heredoc body line that begins with the tool name. Origin: the agent's training matches GNU sed, and on macOS its GNU-shaped `sed` invocations fail against BSD sed.
