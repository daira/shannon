...
**Friction reduction on the noticing→writing path.** The aspirational outcome is *update memory whenever a generalizable issue surfaces, whether the issue is a missed rule, a near-miss, a recurring pattern, or a refinement to an existing rule.* Achieving it is not a matter of the agent being more diligent about noticing; it is a matter of the *path from noticing to writing being short and structurally privileged over the alternative*. The mechanisms doing the work are concrete: this memory-file encodes the rule (so the agent has it loaded); a synthesis-check hook fires on memory writes (so an existing memory is preferred over a new file); the always-loaded `CLAUDE.md` template keeps the rule in scope across compactions. Per `feedback_mechanism_over_intention.md`, naming the mechanisms keeps the goal honest — "the agent should develop a reflex" would be a rhetorical dodge that papers over the work the mechanisms are doing.
