...
#
# Three path cases are recognised:
#
#   `MEMORY.md`      → no output (the index file is not a body memory;
#                      the synthesis question does not apply to index
#                      entries)
#   `user_*.md`      → path-aware reminder — the synthesis half applies
#                      ("is this addition really at home in this user
#                      profile, or does it belong in a feedback memory?")
#                      but the sanitization half is dropped, since named
#                      attribution is the point of user profile memories
#   any other `*.md` → default reminder (synthesis + sanitization)
#
    ...
    *.claude/memory/user_*.md|*.claude/projects/*/memory/user_*.md|*/memory-seed/user_*.md)
        ...
        jq -n '{
            hookSpecificOutput: {
                ...
                additionalContext: "Synthesis check before this user-profile-memory edit: is this addition really at home in this `user_*.md`, or does it belong in a feedback memory? See `feedback_rich_memory_summaries.md` §3 for the synthesis rule. Named attribution is the point of user profile memories, so the sanitization rule in `feedback_external_reports.md` does not apply here."
            }
        }' 2>/dev/null
        ;;
    *.claude/memory/*.md|*.claude/projects/*/memory/*.md|*/memory-seed/*.md)
        # Regular memory body — full reminder. Includes Shannon-side seed
        # sources (`<shannon-checkout>/memory-seed/<name>.md`), so when
        # a maintainer's Claude instance edits the seed source directly,
        # it gets the same synthesis-check coverage as when a user's
        # Claude edits the symlinked or copied install at
